...
from loss import SILogLoss, L1Loss
from utils import RunningAverage, colorize
import matplotlib

from models.swca_depthnet import SWCADepthNet

PROJECT = "SWCA Depth Net"
logging = True
...
def train(model, args, epochs=10, experiment_name="test-01x", lr=0.000359, root=".", device=None, optimizer_state_dict=None):
    global PROJECT
    global backbone_name
    ...
    ###################################### Logging setup #########################################
    print(f"Training {experiment_name} | {backbone_name} | Bins : {args.n_bins}")

    run_id = f"{dt.now().strftime('%d-%h_%H-%M')}-nodebs{args.bs}-tep{epochs}-lr{lr}-wd{args.wd}-{uuid.uuid4()}"
    name = f"{experiment_name}_{backbone_name}_{run_id}"
    ...
    # criterion_ds = DepthSmoothness()
    # criterion_ssim = LossSSIM(args.max_depth)
    ...
    for epoch in range(args.epoch, epochs):
        ...
        for i, batch in tqdm(enumerate(train_loader), desc=f"Epoch: {epoch + 1}/{epochs}. Loop: Train",
            ...
            # print(f"[Original] Image : {img.shape} | Depth : {depth.shape}")
            ...
            # print(img.shape)
            # print(img[0].shape, img[0].min(), img[0].max())

            # pred, bin_edges = model(img) | Alterregress
            pred = model(img) # SWCA depth
            
            pred = torch.clamp(pred, min=args.min_depth, max=args.max_depth)
            # print(f"[Prediction] Depth : {depth.shape} | Pred : {pred.shape}")
            # print(f"[Prediction] Depth range : [{depth.min()} to {depth.max()}]")
            # print(f"[Prediction] Pred range : [{pred.min()} to {pred.max()}]")

            mask = depth > args.min_depth
            mask = mask.to(torch.bool)
            
            l_silog = criterion_silog(pred, depth, mask=mask, interpolate=False)
            # l_berhu = criterion_berhu(pred, depth, mask=mask, interpolate=False) 
            l_l1 = criterion_l1(pred, depth, mask=mask, interpolate=False) 
            ...
            # loss = l_silog + 0.1 * l_chamfer # + l_berhu | Alterregress
            loss = l_silog + l_l1 # l_berhu
            
            # l2_reg = 1e-3 * cum_msa
            # loss = loss_l1 + loss_silog 
            # loss = loss_silog 
            
            loss.backward()
            
            # NYU Use gradient clipping strategy
            nn.utils.clip_grad_norm_(model.parameters(), 0.1)  # optional
            optimizer.step()
            if (step) % 5 == 0:
                # wandb.log({f"Train/{criterion_bins.name}": round(l_chamfer.item(), 5)}, step=step)
                wandb.log({f"Train/{criterion_silog.name}": round(l_silog.item(), 5)}, step=step)
                wandb.log({f"Train/{criterion_l1.name}": round(l_l1.item(), 5)}, step=step)
                # wandb.log({f"Train/{criterion_berhu.name}": round(l_berhu.item(), 5)}, step=step)
                # wandb.log({f"Train/Cumulative MSA": l2_reg.item()}, step=step)
                # wandb.log({f"Train/Double Loss": loss.item()}, step=step)
            
            
            lr_rightnow = scheduler.get_last_lr()
            wandb.log({f"Lr": lr_rightnow[0]}, step=step)
            ...
            if should_write and step % args.validate_every == 0:
                ...
                ################################# Validation loop ##################################################
                model.eval()
                metrics, val_si = validate(args, model, test_loader, [criterion_silog, criterion_l1], epoch, epochs, device)
                
                print(f'metrics : {metrics} | val_si : {val_si}')
                # log_images(img, depth, pred, args, step)

                # print("Validated: {}".format(metrics))
                # if should_log:
                wandb.log({f"Test/DoubleLoss": round(val_si.get_value(), 5) }, step=step)

                wandb.log({f"Test Metrics/{k}": v for k, v in metrics.items()}, step=step)
...
def validate(args, model, test_loader, criterions, epoch, epochs, device='cpu'):
    with torch.no_grad():
        ...
        for batch in tqdm(test_loader, desc=f"Epoch: {epoch + 1}/{epochs}. Loop: Validation") if is_rank_zero(
            ...
            # pred, bin_edges = model(img)
            pred = model(img)
            pred = torch.clamp(pred, min=args.min_depth, max=args.max_depth)
            
            
            mask = depth > args.min_depth
            # mask_ds = torch.clamp(mask.to(torch.float16), min=args.min_depth)
            mask = mask.to(torch.bool)
            
            loss_silog = criterions[0](pred, depth, mask=mask, interpolate=False)
            # l_chamfer = criterions[1](bin_edges, depth)
            loss_berhu = criterions[1](pred, depth, mask=mask, interpolate=False)    
            # loss_ds = criterions[2](pred, depth, mask=mask_ds, interpolate=False)
            ...
            # loss = loss_silog + l_chamfer # + loss_berhu | Alterregress
            loss = loss_silog + loss_berhu # + loss_berhu | Alterregress
...
if __name__ == '__main__':
    ...
    parser.add_argument('--backbone', default='x2_0', type=str, help='The backbone you desire, variants of shufflenet V2')
    parser.add_argument('--attention_type', default='multi_weighted', type=str, help='The backbone you desire, variants of shufflenet V2')
