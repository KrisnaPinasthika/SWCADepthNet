...
def colorize(value, cmap='jet'): # Greys
    # normalize
    import matplotlib.pyplot as plt
    vmin = value.min() # if vmin is None else vmin
    vmax = value.max() # if vmax is None else vmax
    print(f"value before norm : {value.shape} | min : {value.min()}, max :{value.max()}")
    if vmin != vmax:
    ...
    print(f"value after norm : {value.shape} | min : {value.min()}, max :{value.max()}")
    ...
    print(f"Post value after norm : {value.shape} | min : {value.min()}, max :{value.max()}")
    return img
...
def predict_tta(model, image, args):
    ...
    # Todo: SWCA Depth Net
    final = nn.functional.interpolate(torch.Tensor([pred]), (image.shape[-2], image.shape[-1]), mode='bilinear', align_corners=True)
    return torch.Tensor(final)
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(sequential):
            ...
            if args.save_dir is not None:
                ...
                print(f"Final to colorize : {final.shape} | min : {final.min()}, max :{final.max()}")
            ...
            print(f"GT to colorize : {gt.shape} | gt[valid_mask]: {gt[valid_mask].shape} | min : {gt.min()}, max :{gt.max()}")
...
if __name__ == '__main__':
    ...
    model = models.SWCADepthNet(
        device='cuda', 
        backbone_name='eff_b5', 
        window_sizes=window_sizes, # NYU: 5 - KITTI: 11 
